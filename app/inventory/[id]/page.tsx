...
export default function InventorySessionPage({ params }: { params: { id: string } }) {
  ...
  const sorted = useMemo(() => {
    return [...lines].sort((a, b) => {
      const da = a.counted_qty - a.expected_qty;
      const db = b.counted_qty - b.expected_qty;
  ...
  async function addOrIncByEan(scanned: string) {
    setErr(null);
    setMsg(null);

    if (!warehouseId) return setErr("Neviem sklad (warehouse_id).");
    if (status !== "OPEN") return setErr("Inventúra je uzavretá.");

    const eanClean = scanned.trim();
    if (!eanClean) return;
    ...
    // expected_qty zo skladu (stock tabuľka)
    ...
    const expected = Number((st as any)?.qty ?? 0);

    // existuje riadok?
    const existing = lines.find((l) => l.product_id === (prod as any).id);
    const nextCount = (existing?.counted_qty ?? 0) + 1;
    ...
    feedbackOk();
    setMsg(`${(prod as any).name}: ${nextCount}`);
    setEan("");
    await load();
  }
  ...
  async function closeSession() {
    ...
    if (!token) {
      setBusyClose(false);
      return setErr("Nie si prihlásený.");
    }
    ...
    if (!res.ok) return setErr(json?.error ?? json?.details ?? "Close failed");

    setMsg(`Inventúra uzavretá + sklad dorovnaný. Riadkov: ${json.lines ?? 0}`);
    await load();
  }
    ...
    <main style={{ padding: 16 }}>
      ...
      <div style={{ marginTop: 12, padding: 12, background: "#f3f4f6", borderRadius: 12 }}>
        ...
        <div style={{ display: "grid", gap: 10, maxWidth: 520 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button onClick={() => addOrIncByEan(ean)} style={{ padding: "10px 12px", maxWidth: 220 }}>
              Pridať (+1)
            </button>

            <button onClick={() => setEan("")} style={{ padding: "10px 12px" }}>
              Vymazať
            </button>
          </div>
          ...
          <details>
            ...
            <div style={{ marginTop: 10 }}>
              <BarcodeScanner onResult={(txt) => addOrIncByEan(txt)} />
            </div>
          </details>
        </div>
      </div>
      ...
      {status === "OPEN" && (
        <button
          ...
          disabled={busyClose}
          style={{ marginTop: 16, padding: "12px 14px", maxWidth: 320 }}
        >
          {busyClose ? "Uzavieram + dorovnávam…" : "Uzavrieť inventúru + dorovnať sklad"}
        </button>
      )}
    </main>
  );
